...
[![documentationBadge](https://img.shields.io/badge/Documentation-Click%20here-blue?style=for-the-badge)](https://github.com/Extive-eu/ValueSaver#readme)
# ValueSaver
...
if(save.import(123)) console.log(`Imported the ValueSaver with the id 123`);
else console.log(`The ValueSaver with the id 123 does not exist.`);
```

### Saving a ValueSaver with an id
By saving a ValueSaver you will be able to use it again when your NodeJS server was offline for example. You can use the `import` function to use it again. The `save` function requires a custom id.
> Note: If another ValueSaver with the same id already exists, it will push the two ValueSavers together to one ValueSaver.
```js
const {ValueSaver} = require('valuesaver');
const save = new ValueSaver();
save.set(`Some key`, `Some value`);
...
### Removing a save
You can remove a save by saving it or importing it and then use the `removeSave` function.
```js
const {ValueSaver} = require('valuesaver');
const save = new ValueSaver();
save.set(`Some key`, `Some value`);

save.import(123).removeSave();
```
